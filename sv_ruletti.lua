...
RegisterServerEvent('jdev_ruletti:osallistuminen')
AddEventHandler('jdev_ruletti:osallistuminen', function(osallistumismaksu, vari)
	if not arvontakesken then
		...
		if xPlayer.getMoney() >= osallistumismaksu then
			...
			TriggerClientEvent('jdev_ruletti:epaonnistui', _source)
		end
	else
		TriggerClientEvent('esx:showNotification', source, 'Et voi osallistua enää tälle kierrokselle')
		TriggerClientEvent('jdev_ruletti:epaonnistui', source)
	end
end)

RegisterServerEvent('jdev_ruletti:lahella')
AddEventHandler('jdev_ruletti:lahella', function()
	if source ~= nil then
...
function arvonta()
	...
	if lahella1[1] ~= nil then
		for i=1, #lahella1, 1 do
			TriggerClientEvent('jdev_ruletti:tulos', lahella1[i].pelaaja, tulos)
        end
	end
