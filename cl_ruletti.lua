	...
	while true do
		...
		if(GetDistanceBetweenCoords(coords, persex, persey, persez, true) < 10) then
			if(GetDistanceBetweenCoords(coords, persex, persey, persez, true) < 3) then
				if not lahella then
					TriggerServerEvent('jdev_ruletti:lahella')
					lahella = true
				end
				if not arvonta then
					...
					if IsControlJustPressed(0, 38) then
						...
						if osallistumismaksu ~= nil and osallistumismaksu > 0 then
							...
							if vari == 'P' or vari == 'M' or vari == 'V' then
								TriggerServerEvent('jdev_ruletti:osallistuminen', osallistumismaksu, vari)
								liitytty = true
							else
								vari = tonumber(vari)
								if vari ~= nil and vari >= 0 and vari < 37 then
									...
									ESX.ShowNotification('Virheellinen numero / väri!')
								end
							end
						else
...
RegisterNetEvent('jdev_ruletti:tulos')
AddEventHandler('jdev_ruletti:tulos', function(servuntulos)
	tulos = servuntulos
	arvonta = true
	Citizen.Wait(10000)
	arvontavalmis = true
...
RegisterNetEvent('jdev_ruletti:epaonnistui')
AddEventHandler('jdev_ruletti:epaonnistui', function()
	liitytty = false
end)
